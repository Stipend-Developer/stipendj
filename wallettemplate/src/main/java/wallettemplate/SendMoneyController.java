...
public class SendMoneyController {
    ...
    public void initialize() {
        Coin balance = Main.stipend.wallet().getBalance();
    ...
    public void send(ActionEvent event) {
        ...
        try {
            ...
            SendRequest req;
            if (amount.equals(Main.stipend.wallet().getBalance()))
                req = SendRequest.emptyWallet(destination);
            else
                req = SendRequest.to(destination, amount);
            req.aesKey = aesKey;
            sendResult = Main.stipend.wallet().sendCoins(req);
            Futures.addCallback(sendResult.broadcastComplete, new FutureCallback<Transaction>() {
                ...
                public void onFailure(Throwable t) {
                    // We died trying to empty the stipend.
                    crashAlert(t);
                }
            });
            ...
            informationalAlert("Could not empty the stipend",
                    "You may have too little money left in the stipend to make a transaction.");
            overlayUI.done();

...
import org.stipendj.crypto.MnemonicCode;
...
public class WalletSettingsController {
    ...
    public void initialize(@Nullable KeyParameter aesKey) {
        DeterministicSeed seed = Main.stipend.wallet().getKeyChainSeed();
        if (aesKey == null) {
            if (seed.isEncrypted()) {
            ...
            this.aesKey = aesKey;
            seed = seed.decrypt(checkNotNull(Main.stipend.wallet().getKeyCrypter()), "", aesKey);
            // Now we can display the stipend seed as appropriate.
            passwordButton.setText("Remove password");
        }

        // Set the date picker to show the birthday of this stipend.
        ...
        // Don't let the user click restore if the words area contains the current stipend words, or are an invalid set,
    ...
    public void restoreClicked(ActionEvent event) {
        // Don't allow a restore unless this stipend is presently empty. We don't want to end up with two wallets, too
        // much complexity, even though WalletAppKit will keep the current one as a backup file in case of disaster.
        if (Main.stipend.wallet().getBalance().value > 0) {
            informationalAlert("Wallet is not empty",
                    "You must empty this stipend out before attempting to restore an older one, as mixing wallets " +
        ...
        if (aesKey != null) {
            ...
            informationalAlert("Wallet is encrypted",
                    "After restore, the stipend will no longer be encrypted and you must set a new password.");
        }

        log.info("Attempting stipend restore using seed '{}' from date {}", wordsArea.getText(), datePicker.getValue());
        informationalAlert("Wallet restore in progress",
                "Your stipend will now be resynced from the Stipend network. This can take a long time for old wallets.");
        overlayUI.done();
        ...
        // Shut down stipendj and restart it with the new seed.
        Main.stipend.addListener(new Service.Listener() {
            @Override
            public void terminated(Service.State from) {
                Main.instance.setupWalletKit(seed);
                Main.stipend.startAsync();
            }
        }, Platform::runLater);
        Main.stipend.stopAsync();
    }
    ...
    public void passwordButtonClicked(ActionEvent event) {
        if (aesKey == null) {
            ...
            Main.stipend.wallet().decrypt(aesKey);

...
public class Main extends Application {
    ...
    public static WalletAppKit stipend;
    ...
    public void start(Stage mainWindow) throws Exception {
        try {
            // parse command line parameters and initialize stipend
            // --testnet=1
    ...
    private void realStart(Stage mainWindow) throws IOException {
        ...
        // Tell stipendj to run event handlers on the JavaFX UI thread. This keeps things simple and means
        ...
        stipend.addListener(new Service.Listener() {
            @Override
        ...
        scene.getAccelerators().put(KeyCombination.valueOf("Shortcut+F"), () -> stipend.peerGroup().getDownloadPeer().close());
    }

    public void setupWalletKit(@Nullable DeterministicSeed seed) {
        // If seed is non-null it means we are restoring from backup.
        stipend = new WalletAppKit(params, new File("."), getWalletFileName()) {
            @Override
            protected void onSetupCompleted() {
                // Don't make the user wait for confirmations for now, as the intention is they're sending it
                // their own money!
                stipend.wallet().allowSpendingUnconfirmedTransactions();
                Platform.runLater(controller::onBitcoinSetup);
            }
        };

        stipend.setDownloadListener(controller.progressBarUpdater())
               .setBlockingStartup(false)
               .setUserAgent(APP_NAME, "1.0");

        if (seed != null)
            stipend.restoreWalletFromSeed(seed);
    }

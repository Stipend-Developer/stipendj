...
import org.stipendj.crypto.KeyCrypterScrypt;
...
/**
 * User interface for entering a password on demand, e.g. to send money. Also used when encrypting a stipend. Shows a
...
public class WalletPasswordController {
    ...
    @FXML void confirmClicked(ActionEvent event) {
        ...
        final KeyCrypterScrypt keyCrypter = (KeyCrypterScrypt) Main.stipend.wallet().getKeyCrypter();
        checkNotNull(keyCrypter);   // We should never arrive at this GUI if the stipend isn't actually encrypted.
        KeyDerivationTasks tasks = new KeyDerivationTasks(keyCrypter, password, getTargetTime()) {
            ...
            protected final void onFinish(KeyParameter aesKey, int timeTakenMsec) {
                checkGuiThread();
                if (Main.stipend.wallet().checkAESKey(aesKey)) {
    ...
    // Writes the given time to the stipend as a tag so we can find it again in this class.
    public static void setTargetTime(Duration targetTime) {
        ...
        Main.stipend.wallet().setTag(TAG, bytes);
    }
    ...
    public static Duration getTargetTime() throws IllegalArgumentException {
        return Duration.ofMillis(Longs.fromByteArray(Main.stipend.wallet().getTag(TAG).toByteArray()));
    }
}

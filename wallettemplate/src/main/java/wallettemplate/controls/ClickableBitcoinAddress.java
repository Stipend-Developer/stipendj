...
/**
 * A custom control that implements a clickable, copyable Bitcoin address. Clicking it opens a local stipend app. The
    ...
    protected void requestMoney(MouseEvent event) {
        if (event.getButton() == MouseButton.SECONDARY || (event.getButton() == MouseButton.PRIMARY && event.isMetaDown())) {
            ...
            try {
                ...
                GuiUtils.informationalAlert("Opening stipend app failed", "Perhaps you don't have one installed?");
            }
        }
    }

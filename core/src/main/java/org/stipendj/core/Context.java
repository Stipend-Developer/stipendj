...
import org.stipendj.core.listeners.BlockChainListener;
import org.stipendj.store.FlatDB;
import org.stipendj.store.HashStore;
import org.stipendj.store.MasternodeDB;
 ...
 * stipendj for a specific network. You can get an instance of this class through calling {@link #get()}.</p>
 *
...
public class Context {
    ...
    public Context(NetworkParameters params) {
        log.info("Creating stipendj {} context.", VersionMessage.BITCOINJ_VERSION);
     ...
     * independently use stipendj (or possibly alt coin forks of it) can operate correctly.
     *
    ...
    public static Context get() {
        ...
        if (tls == null) {
            if (isStrictMode) {
                log.error("Thread is missing a stipendj context.");
                log.error("You should use Context.propagate() or a ContextPropagatingThreadFactory.");
                throw new IllegalStateException("missing context");
            }
            if (lastConstructed == null)
                throw new IllegalStateException("You must construct a Context object before using stipendj!");
            slot.set(lastConstructed);
            log.error("Performing thread fixup: you are accessing stipendj via a thread that has not had any context set on it.");
    ...
    /**
     * Require that new threads use {@link #propagate(Context)} or {@link org.stipendj.utils.ContextPropagatingThreadFactory},
     ...
     * a {@link org.stipendj.utils.ContextPropagatingThreadFactory}.
     */
    ...
    /**
     * Returns the {@link org.stipendj.core.NetworkParameters} specified when this context was (auto) created. The
     * network parameters defines various hard coded constants for a specific instance of a Stipend network, such as
